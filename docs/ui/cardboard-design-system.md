...
The implemented shell is calibrated to the approved Stitch proportions: `280px` desktop right rail, `80px` tablet rail, `64px` top bar, `24px` desktop gutters, `16px` mobile gutters, `768px` mobile breakpoint, and `1600px` maximum content width. Controls retain `44px` input/touch targets; desktop visual buttons use a compact `40px` height and mobile actions expand to `44–48px`. The review route uses `18/26px` section headings, `12/18px` supporting labels, intrinsic-height cards, compact tables, and a content-sized empty state. The shell grid explicitly uses LTR column ordering so the RTL rail remains physically on the right while its contents remain RTL.
