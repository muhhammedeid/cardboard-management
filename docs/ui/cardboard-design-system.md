...
Desktop is `>=1024px`, tablet `640–1023px`, and mobile `<640px`. Desktop uses the 264 px right rail; tablet collapses it to an icon rail; mobile uses a drawer pattern and mobile action bar. Tables are hidden in favor of record cards at mobile sizes. Focus states, persistent labels, 44 px controls, dialog/drawer keyboard dismissal, semantic disabled/read-only states, and reduced-motion behavior are baseline requirements.
