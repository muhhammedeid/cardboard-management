...
def execute_query(query: Mapping[str, Any]) -> dict[str, Any]:
    ...
    compare_from_dt, compare_to_dt = (None, None)
    if query.get("operation") == "compare":
        compare_from_dt, compare_to_dt = resolve_period_dates(query.get("compare_period"))
    ...
    if query["operation"] == "compare":
        return _run_compare(query, filters, from_dt, to_dt, compare_from_dt, compare_to_dt, supplier_id, item_id)
...
def _run_supplies(query, filters, from_dt, to_dt, supplier_id, item_id):
    ...
    if supplier_id:
        summary, rows = _supply_history_rows(supplier_id, from_dt, to_dt, 500)
        route = _supplier_route(supplier_id)
        weight = flt(sum(flt(row.get("payable_weight")) for row in rows))
        value = flt(sum(flt(row.get("value")) for row in rows))
        aggs = query.get("aggregation") or {}
        facts = []
        if aggs.get("payable_weight_tons") == "sum":
            facts.append(_fact("total.payable_weight_tons", _tons(weight), route, unit="Ton"))
        if aggs.get("payable_weight_kg") == "sum":
            facts.append(_fact("total.payable_weight_kg", weight, route, unit="Kg"))
        if aggs.get("total_amount") == "sum":
            facts.append(_fact("total.total_amount", value, route, currency="EGP"))
        if aggs.get("count") == "count":
            facts.append(_fact("total.count", len(summary.get("supply_history", [])), route))
        if not facts:
            facts = [
                _fact("total.payable_weight_tons", _tons(weight), route, unit="Ton"),
                _fact("total.total_amount", value, route, currency="EGP"),
            ]
        return {"status": "ok", "facts": facts, "records": [], "disambiguation": [], "source_label": "supplier_supply_period_totals"}

    # Period aggregates for supplies as a whole
...
def _comparison_facts(current: Mapping[str, Any], previous: Mapping[str, Any], source: dict[str, str]) -> list[dict[str, Any]]:
    facts = []
    for key in sorted(set(current) & set(previous)):
        cv, pv = current[key], previous[key]
        if isinstance(cv, (int, float)) and not isinstance(cv, bool) and isinstance(pv, (int, float)) and not isinstance(pv, bool):
            facts.append(_fact(f"current.{key}", flt(cv), source))
            facts.append(_fact(f"comparison.{key}", flt(pv), source))
            facts.append(_fact(f"change.{key}", flt(cv) - flt(pv), source))
    return facts


def _run_compare(query, filters, from_dt, to_dt, compare_from_dt, compare_to_dt, supplier_id, item_id):
    domain = query["domain"]
    aggs = query.get("aggregation") or {}
    if domain == "supplies":
        if supplier_id:
            summary_c, rows_c = _supply_history_rows(supplier_id, from_dt, to_dt, 500)
            summary_p, rows_p = _supply_history_rows(supplier_id, compare_from_dt, compare_to_dt, 500)
            current = {
                "payable_weight_kg": flt(sum(flt(row.get("payable_weight")) for row in rows_c)),
                "total_amount": flt(sum(flt(row.get("value")) for row in rows_c)),
                "count": len(rows_c),
            }
            previous = {
                "payable_weight_kg": flt(sum(flt(row.get("payable_weight")) for row in rows_p)),
                "total_amount": flt(sum(flt(row.get("value")) for row in rows_p)),
                "count": len(rows_p),
            }
            route = _supplier_route(supplier_id)
        else:
            current = dict(reporting.get_operations_summary(from_dt, to_dt, item_id).get("supplies", {}))
            previous = dict(reporting.get_operations_summary(compare_from_dt, compare_to_dt, item_id).get("supplies", {}))
            for payload in (current, previous):
                payload["payable_weight_kg"] = payload.get("payable_weight", 0)
            route = {"route": "/supplies", "label": "Supplies"}
        facts = _comparison_facts(current, previous, route)
        facts.append(_fact("current.payable_weight_tons", _tons(current["payable_weight_kg"]), route, unit="Ton"))
        facts.append(_fact("comparison.payable_weight_tons", _tons(previous["payable_weight_kg"]), route, unit="Ton"))
        facts.append(_fact("change.payable_weight_tons", _tons(current["payable_weight_kg"]) - _tons(previous["payable_weight_kg"]), route, unit="Ton"))
        return {"status": "ok", "facts": facts, "records": [], "disambiguation": [], "source_label": "supply_period_comparison"}
    if domain == "sales":
        current = reporting.get_sales_summary(from_dt, to_dt, item_id)
        previous = reporting.get_sales_summary(compare_from_dt, compare_to_dt, item_id)
        keys_present = {"quantity": "total_quantity", "total_amount": "total_informational_value"}
        facts = _comparison_facts(
            {key: flt(current.get(live_key, 0)) for key, live_key in keys_present.items()},
            {key: flt(previous.get(live_key, 0)) for key, live_key in keys_present.items()},
            {"route": "/sales", "label": "Sales"},
        )
        return {"status": "ok", "facts": facts, "records": [], "disambiguation": [], "source_label": "sales_period_comparison"}
    if domain == "expenses":
        current = reporting.get_expense_summary(from_dt, to_dt)
        previous = reporting.get_expense_summary(compare_from_dt, compare_to_dt)
        facts = _comparison_facts(
            {"amount": flt(current.get("total_expense_amount", 0))},
            {"amount": flt(previous.get("total_expense_amount", 0))},
            {"route": "/expenses", "label": "Expenses"},
        )
        return {"status": "ok", "facts": facts, "records": [], "disambiguation": [], "source_label": "expense_period_comparison"}
    if domain == "payments":
        if not supplier_id:
            raise SemanticError("ENTITY_NOT_FOUND", "حدد موردًا لمقارنة دفعاته.")
        current = reporting.get_supplier_summary(supplier_id, from_dt, to_dt)
        previous = reporting.get_supplier_summary(supplier_id, compare_from_dt, compare_to_dt)
        facts = _comparison_facts(
            {"paid_amount": flt(current.get("supplier_payments", 0))},
            {"paid_amount": flt(previous.get("supplier_payments", 0))},
            _supplier_route(supplier_id),
        )
        return {"status": "ok", "facts": facts, "records": [], "disambiguation": [], "source_label": "payment_period_comparison"}
    raise SemanticError("UNSUPPORTED_OPERATION", "المقارنة ممنوعة على النطاق ده.")
